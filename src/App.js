...
function ApiKeyScreen({ onSubmit }) {
  ...
  const handleSubmit = () => {
    ...
    if (!trimmed.startsWith('gsk_')) {
      setErr('Key should start with gsk_ — check your Groq console.');
      return;
    }
    onSubmit(trimmed);
  };
    ...
    <div className="app wizard-screen">
      ...
      <div className="apikey-center">
        ...
        <div className="apikey-box">
          <p className="apikey-label">Enter your Groq API Key to begin</p>
          <p className="apikey-hint">
            Get a free key at{' '}
            <a href="https://console.groq.com/keys" target="_blank" rel="noreferrer" className="apikey-link">
          ...
          <input
            ...
            autoFocus
          />
          {err && <p className="apikey-err">{err}</p>}
          <button className="btn-next" style={{ width: '100%' }} onClick={handleSubmit}>
            Start →
          </button>
          <p className="apikey-note">Your key stays in this browser tab only — never stored or shared.</p>
        </div>
      </div>
...
export default function App() {
  ...
  const callAPI = async () => {
    try {
      ...
      if (!apiKey) throw new Error('No API key — please refresh and enter your key.');
  ...
  /* ── API KEY ────────────────────────────────────── */
  if (screen === 'apikey') {
    return <ApiKeyScreen onSubmit={(key) => {
